...
func TestPlatformService(t *testing.T) {
	...
	t.Run("SignUp", func(t *testing.T) {
		...
		var ss mockSaasService
		ss.On("SignUp", mock.Anything, mock.Anything, &api.SignUpRequest{Email: login, FirstName: "firstName", LastName: "lastName"}).Return(&api.SignUpResponse{}, nil)

		s, err := New(db, &ss)
		require.NoError(t, err)
		s.host = devAuthHost

		err = s.SignUp(context.Background(), login, "firstName", "lastName")
		require.NoError(t, err)
	})
