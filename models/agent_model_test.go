...
func TestAgent(t *testing.T) {
	...
	t.Run("DSN", func(t *testing.T) {
		...
		for typ, expected := range map[models.AgentType]string{
			...
			models.PostgresExporterType:        "postgresql://username:s3cur3%20p%40$$w0r4.@1.2.3.4:12345/database?connect_timeout=1&sslmode=disable",
		} {
	...
	t.Run("DSN ssl", func(t *testing.T) {
		...
		for typ, expected := range map[models.AgentType]string{
			...
			models.PostgresExporterType:        "postgresql://username:s3cur3%20p%40$$w0r4.@1.2.3.4:12345/database?connect_timeout=1&sslcert={{.TextFiles.certificateFilePlaceholder}}&sslkey={{.TextFiles.certificateKeyFilePlaceholder}}&sslmode=verify-ca&sslrootcert={{.TextFiles.caFilePlaceholder}}",
		} {
			t.Run(string(typ), func(t *testing.T) {
				agent.AgentType = typ
				assert.Equal(t, expected, agent.DSN(service, time.Second, "database", nil))
	...
	t.Run("DSN ssl-skip-verify", func(t *testing.T) {
		...
		for typ, expected := range map[models.AgentType]string{
			...
			models.PostgresExporterType:        "postgresql://username:s3cur3%20p%40$$w0r4.@1.2.3.4:12345/database?connect_timeout=1&sslmode=require",
		} {
			t.Run(string(typ), func(t *testing.T) {
				agent.AgentType = typ
				assert.Equal(t, expected, agent.DSN(service, time.Second, "database", nil))
...
func TestPostgresAgentTLS(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		{false, false, "postgresql://username:s3cur3%20p%40$$w0r4.@1.2.3.4:12345/database?connect_timeout=1&sslmode=disable"},
		{false, true, "postgresql://username:s3cur3%20p%40$$w0r4.@1.2.3.4:12345/database?connect_timeout=1&sslmode=disable"},
		{true, false, "postgresql://username:s3cur3%20p%40$$w0r4.@1.2.3.4:12345/database?connect_timeout=1&sslmode=verify-ca"},
		{true, true, "postgresql://username:s3cur3%20p%40$$w0r4.@1.2.3.4:12345/database?connect_timeout=1&sslmode=require"},
	} {
...
func TestPostgresWithSocket(t *testing.T) {
	t.Run("empty-password", func(t *testing.T) {
		...
		expect := "postgresql://username@/database?connect_timeout=1&host=%2Fvar%2Frun%2Fpostgres&sslmode=verify-ca"
	...
	t.Run("empty-user-password", func(t *testing.T) {
		...
		expect := "postgresql:///database?connect_timeout=1&host=%2Fvar%2Frun%2Fpostgres&sslmode=disable"
		assert.Equal(t, expect, agent.DSN(service, time.Second, "database", nil))
	})
	...
	t.Run("dir-with-symbols", func(t *testing.T) {
		...
		expect := "postgresql:///database?connect_timeout=1&host=%2Ftmp%2F123%5C+A0m%5C%25%5C%24%5C%40%5C8%5C%2C%5C%2B%5C-&sslmode=disable"
		assert.Equal(t, expect, agent.DSN(service, time.Second, "database", nil))
	})
}
