...
func TestDatabaseUniqueIndexes(t *testing.T) {
	...
	t.Run("Agents", func(t *testing.T) {
		...
		require.NoError(t, err)
		_, err = db.Exec(
			"INSERT INTO agents (agent_id, agent_type, disabled, status, created_at, updated_at) "+
				"VALUES ('/agent_id/1', 'pmm-agent', false, '', $1, $2)", now, now,
		)
		require.NoError(t, err)

		// runs_on_node_id or pmm_agent_id, not both
		_, err = db.Exec(
			"INSERT INTO agents (agent_id, agent_type, runs_on_node_id, disabled, status, created_at, updated_at) "+
				"VALUES ('/agent_id/2', 'pmm-agent', '/node_id/1', false, '', $1, $2)", now, now,
		)
		require.NoError(t, err)
		_, err = db.Exec(
			"INSERT INTO agents (agent_id, agent_type, pmm_agent_id, disabled, status, created_at, updated_at) "+
				"VALUES ('/agent_id/3', 'mysqld_exporter', '/agent_id/1', false, '', $1, $2)", now, now,
		)
		require.NoError(t, err)
		_, err = db.Exec(
			"INSERT INTO agents (agent_id, agent_type, runs_on_node_id, pmm_agent_id, disabled, status, created_at, updated_at) "+
				"VALUES ('/agent_id/4', 'mysqld_exporter', '/node_id/1', '/agent_id/1', false, '', $1, $2)", now, now,
		)
		assertCheckViolation(t, err, "agents", "runs_on_node_id_or_pmm_agent_id")
	})
}
