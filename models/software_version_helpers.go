...
func (p *CreateServiceSoftwareVersionsParams) Validate() error {
	...
	case MySQLServiceType,
		...
		ExternalServiceType:
	default:
		return NewInvalidArgument("invalid service type %q", p.ServiceType)
	}
	...
	for _, sv := range p.SoftwareVersions {
		...
		default:
			return NewInvalidArgument("invalid software name %q", sv.Name)
		}

		if sv.Version == "" {
			return NewInvalidArgument("empty version for software name %q", sv.Name)
		}
	}
